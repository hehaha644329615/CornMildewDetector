...
| 健康召回率 | 62.4% | **65.8%** | ↑ +3.4% |
| 轻度霉变召回率 | 6.6% | **16.3%** | ↑ +9.7% |
| 重度霉变召回率 | 55.9% | **67.1%** | ↑ +11.2% |
- mAP50 从 0.242 微涨到 0.263，漏检率仍高达 82%

**混淆矩阵（v2，Conf_Thresh=0.3）**：

| 真实类别 | 背景 | 健康 | 轻度霉变 | 重度霉变 | 召回率 |
|:---|:---:|:---:|:---:|:---:|:---:|
| **健康** | 111 | **233** | 4 | 10 | **65.8%** |
| **轻度霉变** | 101 | 48 | **59** | 20 | **16.3%** |
| **重度霉变** | 353 | 23 | 10 | **744** | **67.1%** |
...
1. **轻度霉变仍是最大短板**：召回率仅从 6.6% 提升到 16.3%，仍然有 83.7% 的轻度霉变样本未被正确识别。其中 27.8%（101 张）被误判为背景，13.2%（48 张）被误判为健康，仅 5.5%（20 张）被误判为重度霉变。
   - **注意**：被误判为健康（48 张）的比例甚至超过了被正确识别的数量（59 张），说明轻度霉变样本在特征空间中仍然与健康高度重叠，模型未能有效建立区分边界。

2. **重度霉变改善最明显**：召回率从 55.9% 提升到 67.1%（↑ 11.2%），且误判为轻度和健康的比例进一步降低（合计仅 3%），说明 Focal Loss 让模型对难分类的重度霉变样本投入了更多注意力。

3. **健康样本表现稳定**：召回率 65.8%，且 31.4%（111 张）被误判为背景，与 v1 的 33.6% 基本持平。健康样本的误判模式没有改变——模型仍然倾向于忽略而非误判。

4. **背景误判率有所下降**：重度霉变误判为背景的比例从 41.0% 降至 30.8%（↓ 10.2%），轻度霉变从 55.4% 降至 27.8%（↓ 27.6%）。说明 Focal Loss 确实让模型更努力地去识别这些难分类的正样本。

5. **mAP 绝对值仍然很低**：尽管所有指标都有提升，但 0.263 的 mAP50 距离实际应用仍有巨大差距。轻度霉变 16.3% 的召回率意味着每 6 颗轻度霉变玉米粒就有 5 颗被漏检。


**核心教训**：

Focal Loss 通过降低易分类样本（背景、健康）的损失权重，让模型更关注难分类样本（轻度霉变、重度霉变），确实带来了全面改善。但它无法解决根本问题——**轻度霉变的视觉特征与健康玉米高度重叠，仅靠调整损失函数无法让模型学到有效的区分特征**。

v2 验证了一个重要结论：**算法优化（Loss 设计）只能在数据质量好的前提下锦上添花，无法在数据质量差时雪中送炭**。如果模型连"什么是轻度霉变"的基本视觉模式都学不到，再好的 Loss 也无能为力。

这也直接推动了后续版本的决定：
- **丢掉健康类别** —— 既然健康与轻度霉变难以区分，不如去掉健康类别，让模型只专注于二分类
- **统一拍照标准** —— 减少输入图片的视角、距离、光照差异
